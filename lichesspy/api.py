...
class DefaultApiClient(object):
    ...
    def call(self, path, params=None, post_data=None, auth=None, format=lichesspy.format.JSON, object_type=lichesspy.format.PUBLIC_API_OBJECT):
...
def users_by_team(team, **kwargs):
    ...
    [1349, 1609, ...]
    """
    return _api_get('/api/team/{}/users'.format(team), kwargs, object_type=lichesspy.format.STREAM_OBJECT)
...
def game(game_id, **kwargs):
    ...
    . . K R . . . .
    """
    return _api_get('/game/export/{}'.format(game_id), kwargs, object_type=lichesspy.format.GAME_OBJECT)
...
def games_by_ids_page(ids, **kwargs):
    ...
    """
    return _api_post('/games/export/_ids', kwargs, ','.join(ids), object_type=lichesspy.format.GAME_STREAM_OBJECT)
...
def user_games(username, **kwargs):
    ...
    . . K R . . . .
    """
    return _api_get('/api/games/user/{}'.format(username), kwargs, object_type=lichesspy.format.GAME_STREAM_OBJECT)


def current_game(username, **kwargs):
    """Wrapper for the `GET /api/user/<username>/current-game` endpoint.
    Returns a single game."""
    return _api_get('/api/user/{}/current-game'.format(username), kwargs, object_type=lichesspy.format.GAME_OBJECT)
...
def cloud_eval(fen, **kwargs):
    """Wrapper for the `GET api/cloud-eval <https://lichesspy.org/api#operation/apiCloudEval>`_ endpoint. """
    kwargs['fen'] = fen
    return _api_get('/api/cloud-eval', kwargs)

def login(username, password):
    cookie_jar = _api_post('/login', {'format': lichesspy.format.COOKIES}, {'username': username, 'password': password})
