...
def from_game(game, headers=None):
    ...
    result = '1/2-1/2' if _node(g, 'status') == 'draw' else '1-0' if _node(
        g, 'winner') == 'white' else '0-1' if _node(g, 'winner') == 'black' else '*'
    h = []
    h.append(("Event", "%s %s game" %
              ("Rated" if g["rated"] else "Casual", g["speed"])))
    h.append(('Site', 'https://lichess.org/%s' % g['id']))
    h.append(('Date', datetime.fromtimestamp(
        int(g['createdAt']) / 1000.0).strftime('%Y.%m.%d')))
    h.append(('Round', '?'))
    ...
    if g['speed'] != 'correspondence':
        h.append(('TimeControl', _node(g, 'clock.initial') +
                  '+' + _node(g, 'clock.increment')))
    moves = g['moves']
    pgn = ''
    for i in h:
        key = i[0]
